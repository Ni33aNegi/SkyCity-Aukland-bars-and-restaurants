...
st.set_page_config(page_title="Skycity Aukland Restaurants & Bars", layout="wide", page_icon="🍽️")


st.header(" _ ")
st.title(" Order channel performance and market share analsis for SkyCity Aukland restaurants & bars")
st.header("Dataset:")
...
# ─── Load data ────────────────────────────────────────────────────────────────
st.title("🍽️  Skycity Aukland Restaurant & Bar  —  Analytics Dashboard")

try:
    df = pd.read_csv("Skycity aukland restaurants & bars.csv")
except FileNotFoundError:
    st.error("⚠️  `Skycity aukland restaurants & bars.csv` not found. Place it in the same folder as this script.")
    st.stop()
